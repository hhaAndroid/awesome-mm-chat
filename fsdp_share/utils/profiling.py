...
@contextlib.contextmanager
def maybe_enable_memory_snapshot(enable_snapshot, dump_folder, profile_freq=10, global_step: int = 0):
    if enable_snapshot:
        snapshot_dir = os.path.join(dump_folder, 'mem_profile')
...
def profile_time_and_memory(desc):
    ...
    print0(f"{desc} Elapsed time {cost_time:.2f} seconds, peak gpu memory {max_memory / 1024 ** 3:.1f}G")


def current_max_mem(desc=''):
