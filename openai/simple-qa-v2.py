...
# T采用了不同的分词器，所以参数需要进行调整
TOPK = 1  # 选择数据库里面 topk 条组成 prompt
EXTRA_PADDING_NUM = 1  # 核心参数，对于数据库中被选择的每条参考文本，额外扩展连续的 n 个 item，因为考虑在处理时候会非常多个 item
...
# python simple-qa-v2.py --source ‘你的包括大量 md 文件夹的源文件夹路径，也可以是单个 md 文件’ --embedding-path `保存的 pkl 文件名`
# python simple-qa-v2.py --source /mmyolo/docs/zh_cn


def parse_args():
    parser = argparse.ArgumentParser(description='Simple QA based on GPT-3.5')
    parser.add_argument('--source', help='source dst')
    parser.add_argument('--embedding-path', default='source_embeding_v2.pkl',
...
class QA:
    ...
    def completion(self, query, context):
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {'role': 'system',
                 'content': f'你是一个有帮助的AI文章助手，从下文中提取有用的内容进行回答，不能回答不在下文提到的内容，'
                            f'如果没有相关内容你可以直接回复没有查询到结果，所提供的参考内容的相关性是从高到底排序, 冒号后面的就是参考内容 ：\n\n{text}'},
